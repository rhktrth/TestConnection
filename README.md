...
- IPv4 DNS サーバーへの問い合わせ、ICMP Echo による Ping を実行できます。
- ローカル側には IPv4 アドレスを指定できます。省略した場合は OS の通常の IPv4 送信元選択や待受動作を利用します。
- TCP / UDP / Ping クライアントの接続先には、IPv4 アドレスまたはホスト名 / FQDN を指定できます。
- 複数の TCP / UDP サーバーを同時に起動できます。
- TCP / UDP / DNS / Ping のクライアント試験を登録順に連続実行し、繰り返せます。有限回数を指定した場合は、全試行の完了後にサーバーを含む試験全体を自動停止します。
- 試験設定を CSV で保存・読込みできます。
- 実行結果を画面またはファイルへ出力できます。結果ログの出力先とログファイル名は環境設定から保存できます。
- 試験用に NIC の IPv4 アドレス、サブネットマスク、デフォルトゲートウェイを一時変更し、元の状態へ復元できます。
...
行頭が `#` の行は読込み時にコメントとして扱います。既存ファイルとの互換性のため 6 列目以降がある場合は読み込み時に無視し、保存時は常に 5 列で出力します。CSV の仕様と入力検証は、[`docs/EXTERNAL_DESIGN.md`](docs/EXTERNAL_DESIGN.md) の `EXT-CFG-*` を参照してください。

## NIC 設定変更

NIC の IPv4 アドレス、IPv4 サブネットマスク、IPv4 デフォルトゲートウェイを試験用に一時変更できます。TestConnection は変更前の状態を保持し、手動復元または通常のウィンドウ終了時に元の状態へ戻す設計です。
...
- サーバーで特定のローカル IPv4 アドレスを指定する場合、そのアドレスが対象 PC に設定されている必要があります。
